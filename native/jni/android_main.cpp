// fh2 - Android port (ForzaHorizon2Recomp-Android-RexGlue)
//
...
// (cvar::Init → SDLWindowedAppContext → WindowedApp "fh2" → loop),
// adicionando as pontes específicas de Android:
//   - virtual joystick P1 (overlay Compose → JNI → SDL)
//   - caminho do fh2.toml no armazenamento privado do app
//   - crash handler nativo com backtrace persistido
...
#define ALOG(...) __android_log_print(ANDROID_LOG_INFO, "fh2", __VA_ARGS__)

namespace fh2_android {
...
extern "C" void Fh2CrashHandler(int sig, siginfo_t* info, void*) {
  const char* signame = "?";
  switch (sig) {
  ...
  __android_log_print(ANDROID_LOG_FATAL, "fh2-rex",
  ...
  UnwindState state = {};
  // skip=1: _Unwind_Backtrace começa no chamador (Fh2CrashHandler);
  ...
  for (size_t i = 0; i < state.count; ++i) {
    ...
    if (has_info && dlinfo.dli_sname != nullptr) {
      __android_log_print(ANDROID_LOG_FATAL, "fh2-rex", "  #%02zu  %p  %s + %td",
                          i, state.frames[i], dlinfo.dli_sname,
                          reinterpret_cast<char*>(state.frames[i]) -
                              reinterpret_cast<char*>(dlinfo.dli_saddr));
    } else {
      __android_log_print(ANDROID_LOG_FATAL, "fh2-rex", "  #%02zu  %p  (%s%s)", i,
                          state.frames[i],
...
void InstallCrashHandler(const char* log_file_path, const char* files_dir) {
  ...
  struct sigaction sa = {};
  sa.sa_sigaction = Fh2CrashHandler;
...
void AttachVirtualGamepad() {
  ...
  desc.name = "Forza Horizon 2 Virtual Pad";
...
Java_com_deivid22srk_fh2recomp_game_NativeBridge_nativeGetAbi(JNIEnv* env, jclass) {
...
Java_com_deivid22srk_fh2recomp_game_NativeBridge_nativeSetVirtualPadState(
...
extern "C" std::atomic<uint64_t> g_rexfh2_vk_present_count;

extern "C" JNIEXPORT jlong JNICALL
Java_com_deivid22srk_fh2recomp_game_NativeBridge_nativeGetPresentCount(JNIEnv*, jclass) {
  return static_cast<jlong>(
      g_rexfh2_vk_present_count.load(std::memory_order_relaxed));
}
...
extern "C" std::atomic<int32_t> g_rexfh2_live_fps_cap{-1};
...
Java_com_deivid22srk_fh2recomp_game_NativeBridge_nativeSetFpsCap(JNIEnv*, jclass, jint fps) {
  ...
  if (v > 0 && v < 20) v = 20;
  g_rexfh2_live_fps_cap.store(v, std::memory_order_relaxed);
...
int main(int argc, char** argv) {
  ...
  fh2_android::InstallCrashHandler(log_file_path, app_files_dir);
  ...
  int result;
  {
    ...
    // Cria o app registrado por REX_DEFINE_APP(fh2, ...) no main.cpp do
    // projeto do jogo (recomp/src/main.cpp; XE_UI_WINDOWED_APPS_IN_LIBRARY=1
    // no Android).
    rex::ui::WindowedApp::Creator creator =
        rex::ui::WindowedApp::GetCreator("fh2");
    if (!creator) {
      ALOG("app creator 'fh2' not registered");
      return EXIT_FAILURE;
    }
    std::unique_ptr<rex::ui::WindowedApp> app = creator(app_context);
    ALOG("boot: app 'fh2' (ReXApp) criado");
    ...
    fh2_android::AttachVirtualGamepad();
  ...
  ALOG("fh2 native main exiting with %d", result);
  return result;
}
