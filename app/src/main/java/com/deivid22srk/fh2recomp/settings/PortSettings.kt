/*
 * Configurações REAIS do port Forza Horizon 2 Recomp.
 *
 ...
 *                       em /storage/emulated/0/Forza Horizon 2 Recomp/logs
 ...
 * fh2.toml gerado na hora) — é lá que se confere o que cada campo vira.
 *
 * NOTA (diferenças do port de referência): as opções específicas do motor do
 * port de referência (unlock_all, tex_mods, unlock 60 fps via hook de
 * PresentationInterval) NÃO existem aqui — o código de hooks daquele jogo não
 * faz parte deste port. O limite de FPS segue disponível pelo cvar fps_cap do
 * SDK e os chips 30/60/90/120/∞ do painel rápido continuam funcionando.
 */
package com.deivid22srk.fh2recomp.settings
...
class PortSettingsRepository(context: Context) {
    ...
    private companion object {
        ...
        const val K_VBLANK = "fh2_vblank_hz"
        ...
        const val K_DETAILED_LOGS = "fh2_detailed_logs"
        const val K_PARTICLES = "particles_enabled"
        const val K_REDUCE_MOTION = "reduce_motion"
    }
}
